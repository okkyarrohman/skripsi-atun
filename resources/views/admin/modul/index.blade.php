    ...
    <div class="px-4 pb-14 rounded-lg mt-14">
        <div class="text-gray-900 bg-white gap-5 p-7">
            <div class="flex justify-between items-center p-2">
                ...
                <a href="{{ route('modul-admin.create') }}"
                    ...
                    <tbody>
                        ...
                        @foreach ($moduls as $index => $modul)
                            <tr class="border-b hover:bg-orange-100">
                                ...
                                <td class="p-3 px-5">{{ $modul->judul }}</td>
                                <td class="p-3 px-5">{{ $modul->created_at->format('d/m/Y') }}</td>
                                <td class="px-12 py-4 text-center flex items-center justify-center">
                                    {{-- Show --}}
                                    <a href="{{ route('modul-admin.show', $modul->id) }}"
                                    ...
                                    {{-- Edit --}}
                                    <a href="{{ route('modul-admin.edit', $modul->id) }}"
                                        class="block w-fit mr-3 text-sm bg-custom-orange text-white p-2 rounded focus:outline-none focus:shadow-outline">
                                    ...
                                    {{-- Delete --}}
                                    <form action="{{ route('modul-admin.destroy', $modul->id) }}" method="POST" class="w-fit">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
