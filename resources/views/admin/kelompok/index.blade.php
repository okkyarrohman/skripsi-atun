    ...
    <div class="px-4 pb-14 rounded-lg mt-14">
        <div class="text-gray-900 bg-white gap-5 p-7">
            <div class="flex justify-between items-center p-2">
                ...
                <a href="{{ route('kelompok-admin.create') }}"
                ...
                <table class="w-full text-md bg-white rounded mb-4">
                    <tbody>
                        <tr class="border-b">
                            ...
                            <th class="text-left p-3 px-5">Nama Kelompok</th>
                            <th class="text-left p-3 px-5">Kelas</th>
                            <th class="text-left p-3 px-5">Kuota</th>
                            <th class="text-center p-3 px-5">Aksi</th>
                        </tr>
                        @foreach ($kelompoks as $index => $kelompok)
                            <tr class="border-b hover:bg-orange-100">
                                <td class="p-3 px-5">{{ $index + 1 }}</td>
                                <td class="p-3 px-5">{{ $kelompok->nama }}</td>
                                <td class="p-3 px-5">{{ $kelompok->kelas }}</td>
                                <td class="p-3 px-5">{{ $kelompok->kuota }}</td>
                                <td class="px-12 py-4 text-center flex items-center justify-center">
                                    {{-- Show --}}
                                    <a href="{{ route('kelompok-admin.show', $kelompok->id) }}"
                                    ...
                                    {{-- Edit --}}
                                    <a href="{{ route('kelompok-admin.edit', $kelompok->id) }}"
                                        class="block w-fit mr-3 text-sm bg-custom-orange text-white p-2 rounded focus:outline-none focus:shadow-outline">
                                    ...
                                    {{-- Delete --}}
                                    <form action="{{ route('kelompok-admin.destroy', $kelompok->id) }}" method="POST" class="w-fit">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
