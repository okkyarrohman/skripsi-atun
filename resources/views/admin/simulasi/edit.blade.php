...
<div class="rounded-lg">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="#" class="ms-1 text-lg font-medium text-custom-orange md:ms-2">Edit Simulasi</a>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="rounded-lg">
        <div class="w-full p-7 bg-white rounded-xl flex-col items-start gap-9 inline-flex">
            ...
            <form action="{{ route('simulasi-admin.update', $simulasis->id) }}" method="POST" enctype="multipart/form-data" class="w-full items-start">
                @csrf
                @method('PATCH')
                <div class="mb-5">
                    <label for="judul" class="block mb-2 text-sm font-medium text-gray-900 ">Judul Simulasi</label>
                    <input type="text" id="judul" name="judul" value="{{ $simulasis->judul }}"
                ...
                <div class="mb-5">
                    ...
                    <textarea type="text" id="deskripsi" name="deskripsi"
                        value="{{ $simulasis->deskripsi }}" placeholder="Masukkan Deskripsi Simulasi" rows="7"
                        class="block w-full p-2.5 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-base focus:ring-blue-500 focus:border-blue-500">{{ $simulasis->deskripsi }}</textarea>
                </div>
                <div class="mb-5">
                    <label for="link" class="block mb-2 text-sm font-medium text-gray-900 ">Upload Link Video</label>
                    <input type="text" id="link" name="link" value="{{ $simulasis->link }}"
                        placeholder="Masukkan Link Video Simulasi"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                </div>
                <div class="pt-2 flex justify-end w-full">
                    <a href="{{ route('simulasi-admin.index') }}" class="text-custom-orange border border-custom-orange bg-white focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2">
                        Batal
                    </a>
                    <button type="submit"
